...
public class TitleIndicator extends LinearLayout implements View.OnClickListener,
    ...
    private static final int FOOTER_COLOR = 0xFF76A6EF;

    private static final float FOOTER_TRIANGLE_HEIGHT = 10;//滚动条的高度
    ...
    //选项卡选中状态下的字体颜色
    private ColorStateList mTextSelectColor;
    ...
    private boolean showVLine = true; //是否展示间隔竖线
    private int padding = 0; //设置内边距
    /**
     * Default constructor
     */
    ...
    public TitleIndicator(Context context, AttributeSet attrs) {
        ...
        mTextSelectColor = a.getColorStateList(R.styleable.TitleIndicator_textSelectColor);
...
//            mPerItemWidth = getWidth() / mTotal;
            ...
            scroll_x = (mCurrentScroll-padding - ((tabID) * (getWidth()+padding*2 + mViewPager.getPageMargin()))) / mTotal;
        } else {
        ...
        path.rewind();
        float offset = 0;
        float left_x = mSelectedTab * mPerItemWidth+totalMarginLeft + offset + scroll_x+padding;
        float right_x = (mSelectedTab + 1) * mPerItemWidth+totalMarginLeft- offset + scroll_x+padding;
    ...
    public void setshowVLine(boolean showVLine){
        this.showVLine = showVLine;
    }
    ...
    protected void add(int index, String label, int icon, boolean hasTips) {
        ...
        ivLine = (ImageView) tabIndicator.findViewById(R.id.iv_vertical_line);

        if (index > 0 && showVLine){
            ivLine.setVisibility(INVISIBLE);
        }else {
            ivLine.setVisibility(INVISIBLE);
        }
    ...
    private void setTabTextSize(View tab, boolean selected) {
        ...
        //字体颜色
        //tv.setTextColor(Color.parseColor(selected?"#29a9ff":"#7D7D7D"));
        tv.setTextColor(selected?mTextSelectColor:mTextColor);
    }
