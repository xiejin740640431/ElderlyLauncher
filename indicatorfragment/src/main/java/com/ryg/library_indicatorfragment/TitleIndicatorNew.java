...
public class TitleIndicatorNew extends LinearLayout implements View.OnClickListener,
    ...
    private int perItemWidthType = LAYOUT_WIDTH;
    /**
     * textView集合，用于根据文字长度计算下划线长度
     */
     ...
     * Default constructor
     */
    public TitleIndicatorNew(Context context) {
        super(context);
     ...
     * @param attrs
     */
    public TitleIndicatorNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        setFocusable(true);
        ...
        needLine = a.getBoolean(R.styleable.TitleIndicator_needLine,false);
        ...
        perItemWidthType = a.getInt(R.styleable.TitleIndicator_perItemWidthType,LAYOUT_WIDTH);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int oneWidth = 0;
        if(mTotal>0){
            oneWidth = getWidth() / mTotal;
        }
        if (mTotal != 0) {
//            mPerItemWidth = getWidth() / mTotal;
            int tabID = mSelectedTab;
            mPerItemWidth = getPerItemWidth(tabID);
        ...
        int oneMarginLeft = (oneWidth-mPerItemWidth)/2;
        int totalMarginLeft = oneMarginLeft*(mSelectedTab+1)+oneMarginLeft*mSelectedTab;
        //下面就是如何画线了
        Path path = mPath;
        path.rewind();
        float offset = 0;
        float left_x = mSelectedTab * mPerItemWidth+totalMarginLeft + offset + scroll_x;
        float right_x = (mSelectedTab + 1) * mPerItemWidth+totalMarginLeft - offset + scroll_x;
    ...
    /**
     * 设置下划线长度计算类型
     * @param perItemWidthType
     */
    public void setPerItemWidthType(int perItemWidthType){
        this.perItemWidthType = perItemWidthType;
    }
    /**
     * 根据类型获取线宽度
     * @param tabId
     * @return
     */
    public int getPerItemWidth(int tabId){
        switch (perItemWidthType){
            case LAYOUT_WIDTH:
                return getWidth()/mTotal;
            case TEXT_WIDTH:
                TextView textView = list.get(tabId);
                int textLength = textView.getText().length();
                float width = textView.getPaint().measureText(textView.getText().toString());
                int itemWidth = (int) width/textLength*(textLength+1);
                return itemWidth;
            default:
                return perItemWidthType;
        }
    }
    /**
     * 获取指定下标的选项卡的标题
    ...
    protected void add(int index, String label, int icon, boolean hasTips) {
        ...
        tabIndicator = mInflater.inflate(R.layout.title_flow_indicator_new, this, false);

        final TextView tv = (TextView) tabIndicator.findViewById(R.id.tab_title);
        final ImageView img = (ImageView) tabIndicator.findViewById(R.id.img);
        list.add(tv);
        if (mTextColor != null) {
    ...
    public synchronized void setCurrentTab(int index) {
        ...
        setIconSelected(oldTab,mTabs.get(mSelectedTab),false);

        mSelectedTab = index;
        View newTab = getChildAt(mSelectedTab);
        newTab.setSelected(true);
        setTabTextSize(newTab, true);
        setIconSelected(newTab,mTabs.get(mSelectedTab),true);
        mViewPager.setCurrentItem(mSelectedTab);
        invalidate();
    }

    public void setIconSelected(View tab, TabInfo tabInfo, boolean selected){
        ImageView img = (ImageView) tab.findViewById(R.id.img);
        if(selected){
            if(tabInfo.getIcon_selected()>0)
                img.setImageResource(tabInfo.getIcon_selected());
        }else{
            img.setImageResource(tabInfo.getIcon());
        }
    ...
    private void setTabTextSize(View tab, boolean selected) {
        ...
        //字体颜色
        tv.setTextColor(Color.parseColor(selected?"#fd473c":"#353535"));
    }
