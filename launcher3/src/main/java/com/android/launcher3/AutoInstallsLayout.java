...
import static com.android.launcher3.DefaultLayoutParser.ATTR_URI;
...
public class AutoInstallsLayout {
    ...
    protected static final String ATTR_ICON_RESOURCE_NAME = "iconResourceName";
    protected static final String ATTR_DOWNLOAD_URL = "downloadUrl";
    protected static final String ATTR_SUPPORT_DELETE = "supportDelete";
    ...
    protected int parseAndAddNode(
        ...
        mValues.put(LauncherSettings.Favorites.TITLE, getAttributeValue(parser, ATTR_TITLE));

        mValues.put(LauncherSettings.Favorites.ICON_RESOURCE_NAME, getAttributeValue(parser, ATTR_ICON_RESOURCE_NAME));
        mValues.put(LauncherSettings.Favorites.DOWNLOAD_URL, getAttributeValue(parser, ATTR_DOWNLOAD_URL));
        mValues.put(LauncherSettings.Favorites.SUPPORT_DELETE,getAttributeValue(parser,ATTR_SUPPORT_DELETE));
    ...
    protected long addShortcut(String title, Intent intent, int type) {
        ...
        mValues.put(Favorites.INTENT, intent != null ? intent.toUri(0) : "");
    ...
    protected class AppShortcutParser implements TagParser {
        ...
        public long parseAndAdd(XmlResourceParser parser) {
            ...
            if (!TextUtils.isEmpty(packageName)) {
                mValues.put(LauncherSettings.Favorites.PACKAGE_NAME, packageName);
                if (!TextUtils.isEmpty(className)) {
                    ...
                    return parseAndCheckReommendApp(parser);
                }
            } else {
         ...
         * @return
         */
        public long parseAndCheckReommendApp(XmlResourceParser parser) {
            if (!StringUtils.isEmpty(mValues.getAsString(LauncherSettings.Favorites.PACKAGE_NAME))) {
                try {
                    String packageName = mValues.getAsString(LauncherSettings.Favorites.PACKAGE_NAME);
                    final String uri = getAttributeValue(parser, ATTR_URI);
                    ComponentName cn = ComponentName.unflattenFromString(uri);
                    Intent intent = new Intent(Intent.ACTION_MAIN, null)
                            .addCategory(Intent.CATEGORY_LAUNCHER)
                            .setComponent(cn)
                            .setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                    ResolveInfo conversionRes = mPackageManager.resolveActivity(intent, 0);
                    if (conversionRes != null) {
                        mValues.put(LauncherSettings.Favorites.ICON, BitmapUtil.flattenBitmap(BitmapUtil.drawableToBitamp(conversionRes.loadIcon(mPackageManager))));
                        return addShortcut(mValues.getAsString(LauncherSettings.Favorites.TITLE), intent,
                                LauncherSettings.Favorites.ITEM_TYPE_RECOM_APPLICATION);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
                if (!StringUtils.isEmpty(mValues.getAsString(LauncherSettings.Favorites.DOWNLOAD_URL))) {
                    String iconName = mValues.getAsString(LauncherSettings.Favorites.ICON_RESOURCE_NAME);
                    if (!StringUtils.isEmpty(iconName)) {
                        try {
                            Drawable drawable = mSourceRes.getDrawable(mSourceRes.getIdentifier(iconName, "mipmap", mContext.getPackageName()));
                            mValues.put(LauncherSettings.Favorites.ICON, BitmapUtil.flattenBitmap(BitmapUtil.drawableToBitamp(drawable)));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                    ...
                    return addShortcut(mValues.getAsString(LauncherSettings.Favorites.TITLE), metaIntent,
                            LauncherSettings.Favorites.ITEM_TYPE_RECOM_APPLICATION);
                }
            }
            return invalidPackageOrClass(parser);
    ...
    protected class AppWidgetParser implements TagParser {
        ...
        public long parseAndAdd(XmlResourceParser parser)
            ...
            final String packageName = getAttributeValue(parser, ATTR_PACKAGE_NAME);
            final String className = getAttributeValue(parser, ATTR_CLASS_NAME);
            final String downloadUrl = getAttributeValue(parser, ATTR_DOWNLOAD_URL);
            boolean isRecommendWidget = false;
            ...
            try {
                ...
                try {
                    ...
                    //下载地址若不为空，则标记为推荐应用小部件
                    if (!StringUtils.isEmpty(downloadUrl)) {
                        isRecommendWidget = true;
                    }
                    isAvailable = false;
//                    return -1;
                }
            ...
            long insertedId = -1;
            int appWidgetId;
            int widgetType = Favorites.ITEM_TYPE_APPWIDGET;
            try {
                if(isAvailable){
                ...
                }else if(isRecommendWidget){
                    appWidgetId=0;
                    widgetType = Favorites.ITEM_TYPE_RECOMMEND_APPWIDGET;
                ...
                mValues.put(Favorites.ITEM_TYPE, widgetType);
    ...
    protected static String getAttributeValue(XmlResourceParser parser, String attribute) {
        String value = parser.getAttributeValue(
                "http://schemas.android.com/apk/res-auto/cn.colorfuline.elderlylauncher", attribute);
        if (value == null) {
