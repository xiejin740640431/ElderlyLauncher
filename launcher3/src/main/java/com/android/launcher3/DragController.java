...
public class DragController {
     ...
     * @param dragAction      Coordinates within the bitmap b for the position of item being dragged.
...
//                    }
    ...
    private void drop(float x, float y) {
        ...
        if (dropTarget instanceof DeleteDropTarget && mDragObject.dragInfo != null) {
            if (mDragObject.dragInfo instanceof LauncherAppWidgetInfo) {
                LauncherAppWidgetInfo appWidgetInfo = (LauncherAppWidgetInfo) mDragObject.dragInfo;
                if (appWidgetInfo.supportDelete == LauncherSettings.Favorites.ITEM_SUPPORT_CLEAR) {
                    Intent intent = new Intent(appWidgetInfo.getProviderName().getPackageName() + ".LAUNCHER_CLEAR_APPWIDGET_INFO");
                    intent.putExtra("appWidgetId", appWidgetInfo.getAppWidgetId());
                    mLauncher.sendBroadcast(intent);
                    dropTarget = getWorkspaceTarget();
                }
            }
        }
        mDragObject.x = coordinates[0];
    ...
    public DropTarget getWorkspaceTarget() {
        for (DropTarget dropTarget : mDropTargets) {
